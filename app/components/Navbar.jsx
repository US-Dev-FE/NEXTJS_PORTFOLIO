...
const Navbar = ({isDarkMode, setIsDarkMode}) => {
  ...
  return (
    ...
    <div className={`fixed top-0 right-0 w-11/12 -z-10 translate-y-[-80%] ${isDarkMode ? "hidden" : ""}`}>
      ...
      z-50 ${isScroll ? "bg-white/50 backdrop-blur-lg shadow-sm " : ""} ${isDarkMode ? "dark shadow-white/20" : ""}`} >
        <a href="#top">
            <Image src={isDarkMode ? assets.logo_dark : assets.logo} alt="" className='w-28 cursor-pointer mr-14'/>
        </a>

        <ul className={`hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 ${isScroll ? "" : " bg-white/50 shadow-sm border"} ${isDarkMode ? 'dark border-white/50  bg-transparent' : ''}`}>
        ...
        <div className='flex items-center gap-4'>
            ...
            border border-gray-500 rounded-full ml-4 font-Ovo ${isDarkMode ? 'border-white/50' : ''}` }>Contact
             <Image src={isDarkMode ? assets.arrow_icon_dark : assets.arrow_icon} alt="" className='w-3'/></a>
            <button className='block md:hidden ml-3' onClick={openMenu}>
              <Image src={isDarkMode ? assets.menu_white : assets.menu_black} alt='' className='w-6'/></button>
        </div>
{/* MOBILE MENU */}

      <ul ref={sideMenuRef} className={`flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0
      bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 ${isDarkMode ? 'bg-darkThemeHover text-white' : ''}`}>
